...
const OrderList = () => {
  ...
  const bgDel = "rgba(239, 92, 0, 0.683)";
  ...
  const deleteOrder = (id) => {
    axios
      ...
      .then((response) => {
        ...
        Swal.fire("Supprimé!", "Produit supprimeé.", "success");
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  const renderStatus = (status) => {
    if (status === "en attente")
      return <StyledSpan bgColor={bgSecondary}>{status}</StyledSpan>;
    if (status === "en livraison")
      return <StyledSpan bgColor={bgDel}>{status}</StyledSpan>;
    if (status === "livré")
      return <StyledSpan bgColor={bgGreen}>{status}</StyledSpan>;
  };
                      ...
                      <TableCell align="center">
                        <IconButton>
                          <ModOrder
                            ...
                            isOpen={isOpen}
                            chargerListOrder={chargerListOrder}
                          />
                        </IconButton>
                        <IconButton
                          onClick={() => handleDeleteProduct(List.id)}
                        >
